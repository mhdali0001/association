...
class ExcelImportSeeder extends Seeder
{
    ...
    {
        $jsonPath = base_path('excel_data.json');
        if (!file_exists($jsonPath)) {
            $this->command->error('excel_data.json not found. Run read_excel_fast.php first.');
            return;
        }

        $rows = json_decode(file_get_contents($jsonPath), true);
        $this->command->info('Loaded ' . count($rows) . ' rows from excel_data.json');
        ...
        foreach ($rows as $row) {
            ...
            if ($assocRaw === 'نعم') {
                $otherAssoc = true;
            } elseif ($assocRaw !== '' && $assocRaw !== 'لا') {
                // It's an actual association name
