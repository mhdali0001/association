...
@section('title', 'استيراد بيانات الآيبان — مسالك النور')

@section('breadcrumb')
    <a href="{{ route('payment-review.index') }}" class="hover:text-violet-700 transition-colors">مراجعة بيانات الدفع</a>
    <span class="text-gray-300 mx-1">/</span>
    <span class="text-gray-700">استيراد الآيبان</span>
@endsection
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="flex items-start justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-3">
                <span class="w-9 h-9 rounded-xl bg-violet-100 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-violet-700" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/>
                    </svg>
                </span>
                استيراد بيانات الآيبان من Excel
            </h1>
            <p class="text-sm text-gray-400 mt-1 mr-12">رفع ملف Excel أو CSV لتحديث بيانات الآيبان دفعةً واحدة</p>
        </div>
    ...
    <div class="space-y-4 mb-6">
        ...
        <div class="bg-white border border-emerald-100 rounded-2xl overflow-hidden">
            <div class="bg-emerald-50 border-b border-emerald-100 px-5 py-3 flex items-center gap-2">
                ...
                <span class="text-sm font-semibold text-emerald-700">الأعضاء الذين تم تحديث بياناتهم</span>
            </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
        ...
        <div class="p-6">
            <form method="POST" action="{{ route('payment-review.import.store') }}" enctype="multipart/form-data" class="space-y-5">
                @csrf

                {{-- Target table --}}
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">الجدول المستهدف</label>
                    <div class="flex gap-3">
                        <label class="flex items-center gap-2.5 flex-1 border border-gray-200 rounded-xl px-4 py-3 cursor-pointer hover:border-violet-300 hover:bg-violet-50 transition-all has-[:checked]:border-violet-500 has-[:checked]:bg-violet-50">
                            <input type="radio" name="target" value="payment_info" checked class="text-violet-600 focus:ring-violet-400">
                            <div>
                                <p class="text-sm font-bold text-gray-800">payment_info</p>
                                <p class="text-xs text-gray-500">البيانات الرئيسية للآيبان</p>
                            </div>
                        </label>
                        <label class="flex items-center gap-2.5 flex-1 border border-gray-200 rounded-xl px-4 py-3 cursor-pointer hover:border-violet-300 hover:bg-violet-50 transition-all has-[:checked]:border-violet-500 has-[:checked]:bg-violet-50">
                            <input type="radio" name="target" value="payment_info_ai" class="text-violet-600 focus:ring-violet-400">
                            <div>
                                <p class="text-sm font-bold text-gray-800">payment_info_AI</p>
                                <p class="text-xs text-gray-500">بيانات الآيبان المستخرجة بالذكاء الاصطناعي</p>
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <label for="file-input"
                           class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-200 rounded-2xl cursor-pointer bg-gray-50 hover:bg-violet-50 hover:border-violet-300 transition-all group"
                           id="drop-zone">
                        <svg class="w-10 h-10 text-gray-300 group-hover:text-violet-400 transition-colors mb-3" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/>
                        </svg>
                        <p class="text-sm font-medium text-gray-500 group-hover:text-violet-700 transition-colors" id="drop-label">
                ...
                <div class="flex justify-end">
                    <button type="submit"
                            class="bg-violet-600 hover:bg-violet-700 text-white font-bold text-sm px-7 py-3 rounded-xl shadow-sm hover:shadow-md transition-all flex items-center gap-2">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div class="p-5 space-y-3">
            <p class="text-xs text-gray-500">يجب أن يكون الصف الأول هو صف العناوين. الأعمدة المطلوبة باللون البنفسجي.</p>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                @endphp
                @foreach($cols as $col)
                <div class="flex items-start gap-3 p-3 rounded-xl {{ $col['required'] ? 'bg-violet-50 border border-violet-100' : 'bg-gray-50 border border-gray-100' }}">
                    <span class="w-4 h-4 rounded-full mt-0.5 shrink-0 flex items-center justify-center {{ $col['required'] ? 'bg-violet-500' : 'bg-gray-300' }}">
                        @if($col['required'])
                    ...
                    <div>
                        <div class="flex items-center gap-2 flex-wrap">
                            <code class="text-xs font-mono font-bold {{ $col['required'] ? 'text-violet-800' : 'text-gray-700' }}">{{ $col['ar'] }}</code>
                            <span class="text-gray-300 text-xs">|</span>
                            <code class="text-xs font-mono text-gray-400">{{ $col['en'] }}</code>
                            @if($col['required'])
                                <span class="text-xs bg-violet-100 text-violet-700 rounded px-1.5 py-0.5 font-semibold">مطلوب</span>
                            @endif
                        </div>
            ...
            <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 text-xs text-blue-700 leading-relaxed">
                ...
                <ul class="list-disc list-inside space-y-1 text-blue-600">
                    ...
                    <li>إذا كان العضو موجوداً سيتم تحديث بياناته، وإن لم يكن له سجل سيُنشأ سجل جديد.</li>
                    <li>المسافات في رقم الآيبان تُحذف تلقائياً.</li>
                </ul>
            </div>
        </div>
    </div>
...
<script>
    ...
    ['dragover','dragenter'].forEach(e => {
        dropZone.addEventListener(e, ev => { ev.preventDefault(); dropZone.classList.add('border-violet-400','bg-violet-50'); });
    });
