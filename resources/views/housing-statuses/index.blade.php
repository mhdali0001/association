...
@section('title', 'أوضاع السكن — مسالك النور')

@section('breadcrumb')
    <span class="text-gray-700">أوضاع السكن</span>
@endsection
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">أوضاع السكن</h1>
        <p class="text-sm text-gray-400 mt-0.5">إدارة قائمة أوضاع السكن المرتبطة بالمستفيدين</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
        <div class="bg-gray-50 border-b border-gray-100 px-6 py-3">
            <h2 class="text-sm font-semibold text-gray-700">إضافة وضع سكن جديد</h2>
        </div>
        <div class="p-5">
            <form method="POST" action="{{ route('housing-statuses.store') }}" class="flex gap-3 items-start">
                @csrf
                <div class="flex-1">
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="اسم وضع السكن (مثال: ملك، إيجار، استضافة…)"
                ...
                <div class="shrink-0">
                    <input type="color" name="color" value="{{ old('color', '#059669') }}"
                           class="h-[46px] w-14 border-2 border-gray-300 rounded-xl cursor-pointer p-1 focus:ring-2 focus:ring-emerald-500">
                </div>
                <button type="submit"
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="bg-gray-50 border-b border-gray-100 px-6 py-3 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <span class="text-xs text-gray-400 bg-gray-100 rounded-full px-2.5 py-0.5">{{ $statuses->count() }} وضع</span>
                <span class="text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-full px-2.5 py-0.5">
                    المجموع: {{ number_format($statuses->sum('members_count')) }} عضو
                </span>
            </div>
        </div>
        ...
        @if($statuses->isEmpty())
            <div class="p-10 text-center">
                <p class="text-gray-400 text-sm">لا توجد أوضاع سكن بعد. أضف الأولى أعلاه.</p>
            </div>
        @else
            <ul class="divide-y divide-gray-50">
                ...
                <li class="flex items-center justify-between px-6 py-3.5 hover:bg-gray-50 transition-colors">
                    ...
                    <div class="flex items-center gap-3 view-mode-{{ $status->id }}">
                        <span class="w-4 h-4 rounded-full shrink-0 border border-white shadow-sm"
                              style="background-color: {{ $status->color }}"></span>
                        <a href="{{ route('members.index', ['housing_status_id[]' => $status->id]) }}"
                           class="text-sm font-medium text-gray-800 hover:text-emerald-600 hover:underline transition-colors">
                            {{ $status->name }}
                        </a>
                        <span class="inline-flex items-center justify-center min-w-[24px] h-5 px-1.5 rounded-full text-xs font-bold"
                              style="background:{{ $status->color }}20; color:{{ $status->color }}; border:1px solid {{ $status->color }}40">
                            {{ $status->members_count }}
                        </span>
                        <span class="text-xs font-mono text-gray-400">{{ $status->color }}</span>
                        @if(!$status->is_active)
                    ...
                    {{-- Edit mode --}}
                    <form method="POST" action="{{ route('housing-statuses.update', $status) }}"
                          class="flex items-center gap-2 flex-1 mr-4 hidden edit-mode-{{ $status->id }}">
                        @csrf @method('PUT')
                        ...
                        <input type="color" name="color" value="{{ $status->color }}"
                               class="h-9 w-12 border-2 border-emerald-300 rounded-lg cursor-pointer p-0.5 shrink-0">
                    ...
                    <div class="flex items-center gap-1 actions-{{ $status->id }}">
                        ...
                        <form method="POST" action="{{ route('housing-statuses.destroy', $status) }}"
