...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm mb-6">
    ...
    <div class="flex flex-col gap-3 px-4 py-3 border-b border-gray-100 bg-gray-50/50">
        ...
        <form method="GET" action="{{ route('pending-changes.index') }}" class="flex items-center gap-2 flex-wrap">
            ...
            {{-- Requester select --}}
            <span class="text-xs font-semibold text-gray-500 shrink-0">مقدّم الطلب:</span>
            <select name="requested_by"
                    class="text-sm border border-gray-200 rounded-xl px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-amber-400 transition min-w-[140px]">
                <option value="">الكل</option>
                @foreach($usersList as $u)
                    <option value="{{ $u->id }}" {{ $requestedBy == $u->id ? 'selected' : '' }}>{{ $u->name }}</option>
                @endforeach
            </select>
