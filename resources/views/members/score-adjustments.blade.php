...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm mb-5 overflow-hidden">
    <div class="flex border-b border-gray-100">
        <a href="{{ route('members.score-adjustments', array_merge(request()->except('tab', 'page'), ['tab' => 'deductions'])) }}"
        ...
        <a href="{{ route('members.score-adjustments', array_merge(request()->except('tab', 'page'), ['tab' => 'additions'])) }}"
           class="flex items-center gap-2 px-6 py-3.5 text-sm font-bold border-b-2 transition-colors
            ...
            <div class="flex gap-2">
                <button type="submit"
                        class="flex items-center gap-1.5 text-white text-sm font-bold px-5 py-2 rounded-xl transition-colors
                               {{ $tab === 'additions' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-red-600 hover:bg-red-700' }}">
...
{{-- Table (wrapped in bulk-set form) --}}
@if($members->isEmpty())
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
        <div class="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4
                    {{ $tab === 'additions' ? 'bg-emerald-50' : 'bg-red-50' }}">
            <svg class="w-8 h-8 {{ $tab === 'additions' ? 'text-emerald-300' : 'text-red-300' }}"
        ...
        <p class="text-gray-400 text-sm font-medium">
            {{ $tab === 'additions' ? 'لا يوجد أعضاء مضافة نقاطهم' : 'لا يوجد أعضاء منقوصة نقاطهم' }}
        </p>
    </div>
@else
...
(function () {
    ...
    function bsSetMode(mode) {
        document.getElementById('bs-mode').value = mode;
    ...
    // Initialise mode button state to match current tab
