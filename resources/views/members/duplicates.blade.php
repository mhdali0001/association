...
<div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
    <div>
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900 flex items-center gap-2.5">
            <span class="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-red-100 flex items-center justify-center shrink-0">
        ...
        <p class="text-sm text-gray-400 mt-1 mr-10 sm:mr-12">
            تم العثور على
    ...
    <a href="{{ route('members.index') }}"
       class="self-start sm:self-auto text-sm text-gray-500 hover:text-gray-700 border border-gray-200 hover:border-gray-300 px-4 py-2 rounded-lg transition-colors whitespace-nowrap">
        ← رجوع للأعضاء
    </a>
</div>
        ...
        <div>
            ...
            <select name="verification_status_id" class="w-full border-2 border-gray-200 rounded-xl px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                ...
                @foreach($verificationStatuses as $vs)
                    <option value="{{ $vs->id }}" {{ $verificationId == $vs->id ? 'selected' : '' }}>{{ $vs->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="flex items-end gap-2">
            <button type="submit"
                    class="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold px-5 py-2 rounded-xl transition-colors">
                تطبيق
            </button>
            @if($search || $verificationId || $type !== 'all')
                <a href="{{ route('members.duplicates') }}"
                   class="text-sm text-gray-400 hover:text-gray-600 px-4 py-2 rounded-xl border border-gray-200 hover:border-gray-300 transition-colors whitespace-nowrap">
                    مسح
                </a>
            @endif
        </div>
...
<div class="grid grid-cols-3 gap-3 sm:gap-4 mb-8">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 sm:p-5 flex items-center gap-2 sm:gap-4">
        <div class="w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-red-50 flex items-center justify-center shrink-0">
            <svg class="w-4 h-4 sm:w-5 sm:h-5 text-red-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div class="min-w-0">
            <p class="text-xl sm:text-2xl font-black text-gray-900 leading-none">{{ $byNationalId->count() }}</p>
            <p class="text-xs text-gray-400 mt-0.5 leading-tight">تكرار الهوية</p>
        </div>
    </div>
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 sm:p-5 flex items-center gap-2 sm:gap-4">
        <div class="w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-orange-50 flex items-center justify-center shrink-0">
            <svg class="w-4 h-4 sm:w-5 sm:h-5 text-orange-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
            </svg>
        </div>
        <div class="min-w-0">
            <p class="text-xl sm:text-2xl font-black text-gray-900 leading-none">{{ $byPhone->count() }}</p>
            <p class="text-xs text-gray-400 mt-0.5 leading-tight">تكرار الهاتف</p>
        </div>
    </div>
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 sm:p-5 flex items-center gap-2 sm:gap-4">
        <div class="w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-yellow-50 flex items-center justify-center shrink-0">
            <svg class="w-4 h-4 sm:w-5 sm:h-5 text-yellow-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
            </svg>
        </div>
        <div class="min-w-0">
            <p class="text-xl sm:text-2xl font-black text-gray-900 leading-none">{{ $byName->count() }}</p>
            <p class="text-xs text-gray-400 mt-0.5 leading-tight">تكرار الاسم</p>
        </div>
    </div>
</div>
...
{{-- Reusable member row macro --}}
@php
function memberActions($member) {
    return view('members._duplicate_actions', compact('member'))->render();
}
@endphp
    ...
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-4">
            ...
            <span class="text-xs bg-red-100 text-red-700 rounded-full px-2.5 py-0.5 font-semibold">{{ $byNationalId->count() }} مجموعة</span>
        </div>
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-2xl border border-red-100 shadow-sm overflow-hidden">
                <div class="bg-red-50 border-b border-red-100 px-4 py-2.5 flex items-center justify-between">
                    <div class="flex items-center gap-2 min-w-0">
                        <span class="text-xs font-semibold text-red-600 shrink-0">رقم الهوية:</span>
                        <span class="font-mono text-sm font-bold text-red-700 truncate">{{ $nid }}</span>
                    </div>
                    <span class="text-xs bg-red-100 text-red-700 rounded-full px-2.5 py-0.5 font-semibold shrink-0 mr-2">{{ $group->count() }} سجلات</span>
                </div>
                {{-- Mobile cards --}}
                    ...
                    <div class="p-4">
                        <div class="flex items-start justify-between gap-2 mb-2">
                            <div>
                                ...
                                <p class="text-xs text-gray-400 mt-0.5">ملف: <span class="font-mono">{{ $member->dossier_number ?? '—' }}</span></p>
                            </div>
    ...
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-4">
            ...
            <span class="text-xs bg-orange-100 text-orange-700 rounded-full px-2.5 py-0.5 font-semibold">{{ $byPhone->count() }} مجموعة</span>
        </div>
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-2xl border border-orange-100 shadow-sm overflow-hidden">
                <div class="bg-orange-50 border-b border-orange-100 px-4 py-2.5 flex items-center justify-between">
                    <div class="flex items-center gap-2 min-w-0">
                        <span class="text-xs font-semibold text-orange-600 shrink-0">رقم الهاتف:</span>
                        <span class="font-mono text-sm font-bold text-orange-700 truncate">{{ $phone }}</span>
                    </div>
                    <span class="text-xs bg-orange-100 text-orange-700 rounded-full px-2.5 py-0.5 font-semibold shrink-0 mr-2">{{ $group->count() }} سجلات</span>
                </div>
                {{-- Mobile cards --}}
                <div class="sm:hidden divide-y divide-gray-50">
                    @foreach($group as $member)
                    <div class="p-4">
                        ...
                        <div class="flex items-center gap-3 text-xs text-gray-400 mb-3">
                            <span>هوية: <span class="font-mono">{{ $member->national_id ?? '—' }}</span></span>
                            <span>·</span>
                            <span>{{ $member->created_at->format('Y/m/d') }}</span>
                        </div>
                        <div class="flex items-center gap-3">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-3 mb-4">
            ...
            <span class="text-xs bg-yellow-100 text-yellow-700 rounded-full px-2.5 py-0.5 font-semibold">{{ $byName->count() }} مجموعة</span>
        </div>
        <div class="space-y-4">
            @foreach($byName as $name => $group)
            <div class="bg-white rounded-2xl border border-yellow-100 shadow-sm overflow-hidden">
                <div class="bg-yellow-50 border-b border-yellow-100 px-4 py-2.5 flex items-center justify-between">
                    <div class="flex items-center gap-2 min-w-0">
                        <span class="text-xs font-semibold text-yellow-700 shrink-0">الاسم:</span>
                        <span class="text-sm font-bold text-yellow-800 truncate">{{ $name }}</span>
                    </div>
                    <span class="text-xs bg-yellow-100 text-yellow-700 rounded-full px-2.5 py-0.5 font-semibold shrink-0 mr-2">{{ $group->count() }} سجلات</span>
                </div>
                {{-- Mobile cards --}}
                <div class="sm:hidden divide-y divide-gray-50">
                    @foreach($group as $member)
                    <div class="p-4">
                        <div class="flex items-start justify-between gap-2 mb-2">
                            <div>
                                <p class="text-xs font-mono text-gray-500">هوية: {{ $member->national_id ?? '—' }}</p>
                                <p class="text-xs text-gray-400 mt-0.5">ملف: <span class="font-mono">{{ $member->dossier_number ?? '—' }}</span></p>
                            </div>
                            @if($member->verificationStatus)
                                <span class="text-xs px-2 py-0.5 rounded-full font-medium shrink-0"
                        ...
                        <div class="flex items-center gap-3 text-xs text-gray-400 mb-3">
                            @if($member->marital_status)
                                <span class="px-2 py-0.5 rounded-full bg-purple-50 text-purple-700">{{ $member->marital_status }}</span>
                            @endif
                            <span>{{ $member->created_at->format('Y/m/d') }}</span>
                        </div>
                        <div class="flex items-center gap-3">
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-4 py-2.5">
                                    @if($member->marital_status)
                                        <span class="px-2 py-0.5 rounded-full text-xs bg-purple-50 text-purple-700">{{ $member->marital_status }}</span>
                                    @else <span class="text-gray-300 text-xs">—</span>
                                    @endif
                                </td>
                                <td class="px-4 py-2.5">
