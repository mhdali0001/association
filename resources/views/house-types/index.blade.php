...
@section('title', 'أنواع البيوت — مسالك النور')

@section('breadcrumb')
    <span class="text-gray-700">أنواع البيوت</span>
@endsection
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">أنواع البيوت</h1>
        <p class="text-sm text-gray-400 mt-0.5">إدارة قائمة أنواع البيوت المستخدمة في الجولات الميدانية</p>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
        <div class="bg-gray-50 border-b border-gray-100 px-5 py-3">
            <h2 class="text-sm font-semibold text-gray-700">إضافة نوع جديد</h2>
        </div>
        <div class="p-5">
            <form method="POST" action="{{ route('house-types.store') }}">
                @csrf
                <div class="flex flex-col sm:flex-row gap-3">
                    <div class="flex-1">
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="اسم النوع (مثال: شقة، منزل مستقل، غرفة…)"
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="bg-gray-50 border-b border-gray-100 px-5 py-3 flex items-center justify-between">
            ...
            <span class="text-xs text-gray-400 bg-gray-100 rounded-full px-2.5 py-0.5">{{ $types->count() }} نوع</span>
        </div>

        @if($types->isEmpty())
            <div class="p-10 text-center">
                <p class="text-gray-400 text-sm">لا توجد أنواع بعد.</p>
            </div>
        @else
            <div class="divide-y divide-gray-100">
                ...
                <div class="px-5 py-4">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="w-5 h-5 rounded-full shrink-0 ring-2 ring-offset-2" style="background:{{ $type->color }}; ring-color:{{ $type->color }}"></div>
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                <span class="font-semibold text-sm text-gray-900">{{ $type->name }}</span>
                                @if(!$type->is_active)
                                    <span class="text-xs text-gray-400 bg-gray-100 rounded-full px-2 py-0.5">معطّل</span>
                                @endif
                            </div>
                            <span class="text-xs text-gray-400">{{ number_format($type->field_visits_count) }} جولة</span>
                        </div>
                        <div class="flex gap-1 shrink-0">
                            ...
                            <form method="POST" action="{{ route('house-types.destroy', $type) }}"
                                  onsubmit="return confirm('حذف النوع: {{ $type->name }}؟')">
                                @csrf @method('DELETE')
                    ...
                    <div id="edit-{{ $type->id }}" class="hidden mt-4 pt-4 border-t border-gray-100">
                        <form method="POST" action="{{ route('house-types.update', $type) }}">
                            @csrf @method('PUT')
                            <div class="flex flex-col gap-3">
                                <div>
                                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">اسم النوع</label>
