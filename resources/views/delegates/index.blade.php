...
@else
    ...
    <div class="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="divide-y divide-gray-50">
            ...
            <div class="flex items-center gap-4 px-5 py-4 hover:bg-sky-50/40 transition-colors group">

                {{-- Rank --}}
                <span class="w-7 text-center text-xs font-bold text-gray-300 shrink-0">{{ $index + 1 }}</span>
                ...
                {{-- Name + bar --}}
                <a href="{{ route('delegates.show', $delegate->delegate) }}" class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-1 shrink-0">
                    <button type="button"
                            onclick="openRename({{ json_encode($delegate->delegate) }}, '{{ route('delegates.rename', $delegate->delegate) }}')"
                    ...
                    <form method="POST" action="{{ route('delegates.destroy', $delegate->delegate) }}"
                          onsubmit="return confirm('هل أنت متأكد من إزالة المندوب «{{ addslashes($delegate->delegate) }}» من {{ $delegate->members_count }} عضو؟')">
                        @csrf @method('DELETE')
                        <button type="submit"
        ...
        <form id="rename-form" method="POST" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="new_name" id="rename-new-name" required
                       class="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:ring-2 focus:ring-sky-400 focus:outline-none bg-gray-50">
            </div>
